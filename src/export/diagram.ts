...
export function svgDiagram(input:Project,viewId=input.rootViewId,metadata=true,icons:IconData=noIcons):string{
 ...
 // Lines first, then labels with a background halo, so a label stays readable where another connection passes.
 const lines=scene.edges.map(e=>`<path d="${svgPolylinePath(e.points)}" fill="none" stroke="#6d7f96" stroke-width="2" ${e.dashed?'stroke-dasharray="5 5"':''} marker-end="url(#arrow)"/>`).join('')+scene.edges.map(e=>e.label?svgText(e.label,' paint-order="stroke" stroke="#f8fafc" stroke-width="4" stroke-linejoin="round"'):'').join('');
