...
export function itemBody(p:ExperiencePack,i:ExperienceItem,opts:RenderOptions={}):string{
 switch(i.type){
  ...
  case 'kpi':{const tone=i.tone??(i.trend==='up'?'good':i.trend==='down'?'bad':'neutral'),k=kpiDisplay(p,i);return `<div class="xp-kpi-tile"><div class="xp-kpi"><span class="xp-kpi-value">${escape(k.value)}</span> <small>${escape(i.unit)}</small></div>${i.delta||i.trend?`<div class="xp-delta xp-${tone}">${i.trend?`<span aria-hidden="true">${ARROW[i.trend]}</span> `:''}${escape(i.delta??'')}${i.comparison?` <span class="xp-muted">${escape(i.comparison)}</span>`:''}</div>`:''}${k.note?`<p class="xp-kpi-note">${escape(k.note)}</p>`:''}${k.from?`<small class="xp-kpi-from">↻ counted from “${escape(k.from)}”</small>`:''}</div>`;}
...
export function itemSvg(p:ExperiencePack,i:ExperienceItem):string{
 ...
 const lines=i.type==='code'?i.code.split('\n'):i.type==='table'?[i.columns.join(' | '),...i.rows.map(r=>r.join(' | '))]:i.type==='kpi'?(()=>{const k=kpiDisplay(p,i);return [`${k.value} ${i.unit}`,[i.trend?ARROW[i.trend]:'',i.delta,i.comparison].filter(Boolean).join(' '),k.note,k.from?`Counted from ${k.from}`:''];})()
...
.xp-kpi-tile{display:flex;flex-direction:column;gap:4px}.xp-kpi{font-size:30px;font-weight:700;line-height:1.1;color:#13294a;letter-spacing:-.5px}.xp-kpi small{font-size:13px;font-weight:500;color:#5b6f88;letter-spacing:0}.xp-delta{font-size:12px;font-weight:600}.xp-delta.xp-good{color:#16774f}.xp-delta.xp-bad{color:#b3263b}.xp-delta.xp-neutral{color:#5a6b80}.xp-muted{color:#6b7b90;font-weight:400}.xp-kpi-from{font-size:10.5px;color:#6b7b90}.xp-kpi-note{font-size:11px;color:#6b7b90;margin:4px 0 0}
