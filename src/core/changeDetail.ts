...
export const GROUP_LABEL:Record<string,string>={project:'Project',nodes:'Components',edges:'Connections',views:'Views',blocks:'Evidence',assets:'Images',sources:'Sources',story:'Story steps',
...
const SINGULAR:Record<string,string>={nodes:'component',edges:'connection',views:'view',blocks:'evidence block',assets:'image',sources:'source',story:'story step',
...
const nameOf=(x:Rec)=>String(x.label??x.title??x.name??x.id);
...
function diffGroup(group:string,current:Rec[],incoming:Rec[],out:ChangeDetail){
 ...
 for(const id of d.added){const x=d.after.get(id)!;out.changes.push({kind:'added',group,id,name:nameOf(x),fields:[]});
...
const PROJECT_FIELDS=['title','summary','author','category','tags','rootViewId','provenance','privateNotes'] as const;
...
export function describeChanges(current:Project,incoming:Project):ChangeDetail{
 ...
 for(const g of ['nodes','edges','views','blocks','assets','sources'] as const)diffGroup(g,current[g] as unknown as Rec[],incoming[g] as unknown as Rec[],out);
