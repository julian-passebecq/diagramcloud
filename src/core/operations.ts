...
export function publicDocument(input:Project):Project{
 ...
 for(const v of d.views){v.nodeIds=v.nodeIds.filter(id=>keptNodes.has(id));v.edgeIds=v.edgeIds.filter(id=>edges.has(id));v.positions=Object.fromEntries(Object.entries(v.positions).filter(([id])=>v.nodeIds.includes(id)));}
 ...
 const blocks=new Set(d.blocks.map(b=>b.id));d.nodes.forEach(n=>n.blockIds=n.blockIds.filter(id=>blocks.has(id)));const assetRefs=new Set(d.blocks.flatMap(b=>b.type==='image'?[b.assetId]:[]));d.assets=d.assets.filter(a=>assetRefs.has(a.id));d.assets.forEach(asset=>{delete asset.remote;});
 ...
 d.story=d.story.filter(s=>keptViews.has(s.viewId)&&(!s.nodeId||keptNodes.has(s.nodeId))).map(s=>({...s,highlightEdgeIds:s.highlightEdgeIds.filter(id=>edges.has(id))}));return validateDocument(d);
}
