...
export function createLatestSaveQueue<T>(save:(value:T)=>Promise<void>,report:(state:SaveQueueState)=>void){
 let tail:Promise<void>=Promise.resolve(),generation=0,latestSaved=true;
 return{
  enqueue(value:T):Promise<void>{
   const mine=++generation;latestSaved=false;report({status:'saving'});
   tail=tail.then(()=>save(value),()=>save(value)).then(
    ()=>{if(mine===generation){latestSaved=true;report({status:'saved'});}},
  ...
  /** True while the newest snapshot is queued, saving or failed: closing the tab now would lose it. */
  unsaved():boolean{return !latestSaved;}
 };
}
