...
test('reduced motion and Fabric icon loading',async({browser})=>{
 ...
 await expect.poll(()=>page.locator('.official-icon').evaluate((img:HTMLImageElement)=>img.complete&&img.naturalWidth>0)).toBe(true);await expect(page.locator('.official-icon').first()).toHaveAttribute('title','Microsoft Fabric Lakehouse · Microsoft artwork, used under the Microsoft Fabric icons usage terms');await page.screenshot({path:'test-results/showcase-fabric.png',fullPage:true});await context.close();
});
